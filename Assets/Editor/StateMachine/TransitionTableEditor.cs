...
using System.Linq;
using Editor.StateMachine.Utilities;
using UnityEditor;
using UnityEngine;
...
{
	[CustomEditor(typeof(StateTransitionTableSO))]
	internal class TransitionTableEditor : UnityEditor.Editor
	{
		...
		private void CopyConditions(SerializedProperty copyTo, SerializedProperty copyFrom)
		{
			for (int i = 0, j = copyTo.arraySize; i < copyFrom.arraySize; i++, j++)
			{
				...
				cond.FindPropertyRelative("ConditionSO").objectReferenceValue = srcCond.FindPropertyRelative("ConditionSO").objectReferenceValue;
			}
		}
