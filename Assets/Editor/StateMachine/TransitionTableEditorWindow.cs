...
{
	public class TransitionTableEditorWindow : EditorWindow
	{
		private static TransitionTableEditorWindow _window;
		private static readonly string _uxmlPath = "Assets/Editor/StateMachine/TransitionTableEditorWindow.uxml";
		private static readonly string _ussPath = "Assets/Editor/StateMachine/TransitionTableEditorWindow.uss";
		private bool _doRefresh;

		private UnityEditor.Editor _transitionTableEditor;

		[MenuItem("State Machine Editor", menuItem = "Custom Editor/State Machine Editor")]
		...
		private void OnListSelectionChanged(IEnumerable<object> enumerable)
		{
			...
			var table = (StateTransitionTableSO)list[0];
			if (table == null)
				return;
		...
		private StateTransitionTableSO[] FindAssets()
		{
			var guids = AssetDatabase.FindAssets($"t:{nameof(StateTransitionTableSO)}");
			var assets = new StateTransitionTableSO[guids.Length];
			for (int i = 0; i < guids.Length; i++)
				assets[i] = AssetDatabase.LoadAssetAtPath<StateTransitionTableSO>(AssetDatabase.GUIDToAssetPath(guids[i]));
